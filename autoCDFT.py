...
def coefficient(atom, info, a):
    ''' return the orbital of major contribution in an **atom**
    search in **info** the biggest coefficient of the orbital **a** in an **atom**
    **a** is the column of the orbital in **info**
    '''
    orbital_coefficient = []
    for i in atom:
            for j in info:
                if (j.split()[1] == i):
                    d = info.index(j)
                if (j.split()[1] == str(int(i) + 1)):
                    e = info.index(j)
            major = float(info[d + 1].split()[a])
            for k in info[d + 1:e]:
                if (major*major < float(k.split()[a])*float(k.split()[a])):
                    major = float(k.split()[a])
                    orbital = k.split()[1]
            b = 5 - len(orbital)
            if (major > 0):
                c = 1
            else: c = 0
            orbital_coefficient.append(orbital + b*' ' + '          ' + c*' ' + str(major))
    return orbital_coefficient
